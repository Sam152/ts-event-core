...
>({ aggregateRoots, aggregateRootRepository }: {
  ...
  const command = (await getTxn()<QueuedCommand[]>`
  ...
  await getTxn()`UPDATE event_core.command_queue SET status = 'complete' WHERE id = ${command.id}`;
}
