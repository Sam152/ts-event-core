 ...
 * the domain.
 */
export function withScheduledFlight<TCommandFunc extends CommandThatHandlesScheduledFlights>(commandName: string, command: TCommandFunc): DecoratedCommand<TCommandFunc> {
    return ((...args: Parameters<TCommandFunc>) => {
        if (args[0] === undefined) {
            return {
                type: 'COMMAND_FAILED',
                commandName,
